...
function source:enabled()
  return (vim.api.nvim_get_current_line():match('^tags') ~= nil) and
...
local function collect_segments()
  ...
  vim.iter(vim.fn.getline(1, '$'))
      :filter(function(l) return l:match('^tags') end)
      :map(function(s) return vim.split(s, '%s+') end)
      :flatten()
      :filter(function(s) return s:match(':') end)
      :each(function(tag)
...
function source:get_completions(ctx, callback)
  ...
  callback {
    ...
    items = vim.iter(collect_segments())
        :map(function(s) return make_item(s, ctx, segment_start) end)
        :totable(),
  }
  return function() end
end
